...
| **PHOTOGRAPHER** | Onboarding status (`/photographer/onboarding-status`), portfolio management (`/photographer/portfolio`), availability blocking (`/photographer/schedule`), incoming requests (`/photographer/bookings`), accept/reject/complete bookings, booking chat (`/bookings/{id}/chat`), and public customer reviews. | Cannot access admin routes (`/admin/**`), cannot pay deposits or access customer deposit checkout, cannot modify another photographer's portfolio/availability/bookings. |
